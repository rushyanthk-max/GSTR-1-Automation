...
# =========================================================================
st.sidebar.header("⚙️ Dynamic Mapping Settings")
st.sidebar.write("The engine automatically matches columns, but you can review the console alerts if a non-standard layout is uploaded.")
        ...
        if attribute_file:
            ...
            if attr_hsn_col and attr_sku_col:
                for _, row in attr_df.iterrows():
                    r_hsn = str(row[attr_hsn_col]).strip()
                    r_sku = str(row[attr_sku_col]).strip()
